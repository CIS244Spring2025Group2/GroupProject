...
public class PlantReminderController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        reminderTypeComboBox.setItems(FXCollections.observableArrayList(
            "Water", "Fertilize", "Repot", "Move", "Harvest"
        ));
        
        // Initialize plant options
        plantComboBox.setItems(FXCollections.observableArrayList(
            "Monstera", "Orchid", "Snake Plant", "Fiddle Leaf Fig", "Basil", 
            "Pothos", "ZZ Plant", "Peace Lily", "Spider Plant", "Aloe Vera"
        ));
        
        // Initialize interval options
        intervalComboBox.setItems(FXCollections.observableArrayList(
        ...
        reminderTypeComboBox.getSelectionModel().selectedItemProperty().addListener(
            (obs, oldVal, newVal) -> {
                if (newVal != null) {
        ...
        try {
            switch(selectedType) {
                ...
                case "Fertilize":
                    ...
                    reminder = new FertilizeReminder(plantName, formattedDate, 
                                                    fertilizerField.getText(), fertAmount);
                    break;
                ...
                case "Repot":
                    ...
                    reminder = new RepotReminder(plantName, formattedDate, 
                                               potSizeField.getText(), soilTypeField.getText());
                    break;
                    
                case "Move":
                    TextField locationField = (TextField) dynamicFields.get("newLocation");
                    TextField reasonField = (TextField) dynamicFields.get("reason");
                    reminder = new MoveReminder(plantName, formattedDate, 
                                              locationField.getText(), reasonField.getText());
                    break;
                    
                case "Harvest":
                    TextField partField = (TextField) dynamicFields.get("harvestPart");
                    TextField useField = (TextField) dynamicFields.get("useFor");
                    reminder = new HarvestReminder(plantName, formattedDate, 
                                                 partField.getText(), useField.getText());
                    break;
            }
